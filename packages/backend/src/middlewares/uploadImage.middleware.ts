...
export const handleImageUpload = (model: 'user' | 'message') => async (req: Request, res: Response, next: NextFunction) => {
  switch (model) {
    case 'user':
			...
			if (!user) return res.status(httpStatus.NOT_FOUND).json({ status: 'fail', message: 'User not found' });
    ...
    default:
      return res.status(httpStatus.NOT_FOUND).json({ status: 'fail', message: 'Invalid model' });
  }
};
...
function handleImage(uploadFunction: Multer, imageUrl: string | null) {
  return async function (req: Request, res: Response, next: NextFunction) {
    uploadFunction.single('image')(req, res, async function(err: any) {
      if (err || !req.file) return res.status(httpStatus.NOT_FOUND).json({ status: 'fail', message: 'NoImageUploaded, FileTooLarge or FileTypeNotSupported.' });
